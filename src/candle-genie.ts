...
import { CandleGenieBot } from "./bot/candlegenie-bot";
import { CandleGeniePredictionV3__factory } from "./types/typechain";

/**
 * Main entry point for CandleGenie Prediction Bot
 */
async function main(): Promise<void> {
  try {
    ...
    const contract = CandleGeniePredictionV3__factory.connect(
      CONTRACT_ADDRESSES.CANDLEGENIE_V3,
      signer
    );

    const strategy = parseStrategy(process.argv);
    const bot = new CandleGenieBot(config, contract);
    bot.start(strategy);
  } catch (error) {
